...
from matris.model.functions import _CudaFusedLineAttention  # noqa: E402
...
P108_LOCAL_ATTENTION_ENV = {
    ...
    "MATRIS_P83C_LINE_ATTENTION_NODE_INPUT": "0",
...
@dataclass
class AttentionMacroCase:
    case_id: int
    kind: str
    rows: int
    num_segments: int
    node_feat: torch.Tensor
    edge_feat0: torch.Tensor
    values: torch.Tensor
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Local torch.compile experiment for a wider attention macro: "
            "alpha_projection + attention_reduce + node_update_input_concat."
        )
    )
    ...
    parser.add_argument("--output-json", default="results/compile_local_attention_macro_limit6_cases60_20260526.json")
...
def torch_attention_reduce_template(
    ...
    source_out, _ = torch_attention_side_template(source_logits, values, source_index, out_template)
    target_out, _ = torch_attention_side_template(target_logits, values, target_index, out_template)
    return source_out, target_out


def torch_attention_macro_template(
    node_feat: torch.Tensor,
    edge_feat0: torch.Tensor,
    values: torch.Tensor,
    source_weight: torch.Tensor,
    ...
    source_logits = edge_feat0.matmul(source_weight.t())
    target_logits = edge_feat0.matmul(target_weight.t())
    source_out, target_out = torch_attention_reduce_template(
        source_logits,
        target_logits,
        values,
        source_index,
    ...
    return torch.cat([node_feat, target_out, source_out], dim=1)


def torch_attention_macro_eval_template(*args) -> torch.Tensor:
    return torch_attention_macro_template(*args)


def torch_attention_macro_grad_template(*args) -> torch.Tensor:
    return torch_attention_macro_template(*args)


def cuda_fused_attention_macro_template(
    node_feat: torch.Tensor,
    edge_feat0: torch.Tensor,
    values: torch.Tensor,
    source_weight: torch.Tensor,
...
def autograd_grads(
    ...
    node_feat_base: torch.Tensor,
    edge_feat0_base: torch.Tensor,
    ...
    *,
    atom_graph: bool | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    node_feat = node_feat_base.detach().requires_grad_(True)
    edge_feat0 = edge_feat0_base.detach().requires_grad_(True)
    ...
    return torch.autograd.grad(out, (node_feat, edge_feat0, values), grad_outputs=grad_out, allow_unused=False)


class AttentionMacroCaseCollector:
    def __init__(self, *, max_cases: int, case_kind: str, max_rows: int) -> None:
        self.max_cases = max_cases
        self.case_kind = case_kind
        self.max_rows = max_rows
        self.cases: list[AttentionMacroCase] = []
        self.skipped: dict[str, int] = {}
        self._original_attention = interaction_block.fused_line_attention_or_none
        self._original_forward = interaction_block.Graph_Attention_Layer.forward
        self._context_stack: list[dict[str, Any]] = []

    def __enter__(self):
        collector = self

        def forward_wrapped(module, node_feat, edge_feat, graph, directed2undirected=None):
            collector._context_stack.append(
                {
                    "module": module,
                    "node_feat": node_feat,
                    "edge_feat": edge_feat,
            ...
            try:
                return collector._original_forward(module, node_feat, edge_feat, graph, directed2undirected)
            finally:
                collector._context_stack.pop()
        ...
        def attention_wrapped(
            ...
            *,
            ...
            result = collector._original_attention(
                source_logits,
                target_logits,
                values,
                source_index,
        ...
        interaction_block.Graph_Attention_Layer.forward = forward_wrapped
        interaction_block.fused_line_attention_or_none = attention_wrapped
        return self

    def __exit__(self, exc_type, exc, tb) -> None:
        interaction_block.fused_line_attention_or_none = self._original_attention
        interaction_block.Graph_Attention_Layer.forward = self._original_forward
    ...
    def _maybe_record(
        ...
        if not self._context_stack:
            self._skip("missing_attention_context")
            return
        ...
        if values.ndim != 2 or values.shape[1] != 128:
            self._skip("unsupported_value_shape")
            return
        ...
        module = ctx["module"]
        node_feat = ctx["node_feat"]
        edge_feat = ctx["edge_feat"]
        directed2undirected = ctx["directed2undirected"]
        if not isinstance(node_feat, torch.Tensor) or not isinstance(edge_feat, torch.Tensor):
            self._skip("bad_context_tensors")
            return
        if atom_graph:
            if not isinstance(directed2undirected, torch.Tensor):
                self._skip("missing_directed2undirected")
                return
            edge_feat0 = torch.index_select(edge_feat, 0, directed2undirected)
        else:
            edge_feat0 = edge_feat
        if edge_feat0.shape != values.shape:
            self._skip("edge_feat0_value_shape_mismatch")
            return
        ...
        self.cases.append(
            AttentionMacroCase(
                ...
                node_feat=node_feat.detach().float().cpu().contiguous(),
                edge_feat0=edge_feat0.detach().float().cpu().contiguous(),
                values=values.detach().float().cpu().contiguous(),
                source_weight=module.source_weight_linear.weight.detach().float().cpu().contiguous(),
                target_weight=module.target_weight_linear.weight.detach().float().cpu().contiguous(),
...
def collect_cases(args: argparse.Namespace) -> tuple[list[AttentionMacroCase], dict[str, Any]]:
    ...
    errors: list[str] = []
    with AttentionMacroCaseCollector(max_cases=args.max_cases, case_kind=args.case_kind, max_rows=args.max_rows) as collector:
        for idx in tqdm(range(len(keys)), desc="collect attention macro cases", leave=False):
...
def case_to_device(case: AttentionMacroCase, device: torch.device) -> dict[str, Any]:
    return {
        "case_id": case.case_id,
        "kind": case.kind,
        "rows": case.rows,
        "num_segments": case.num_segments,
        "node_feat": case.node_feat.to(device),
        "edge_feat0": case.edge_feat0.to(device),
        "values": case.values.to(device),
        "source_weight": case.source_weight.to(device),
        "target_weight": case.target_weight.to(device),
        "source_index": case.source_index.to(device),
        "target_index": case.target_index.to(device),
...
def benchmark_case(
    ...
    compiled_grad: Callable[..., torch.Tensor],
    case: AttentionMacroCase,
    args: argparse.Namespace,
) -> dict[str, Any]:
    device = torch.device(args.device)
    item = case_to_device(case, device)
    node_feat = item["node_feat"]
    edge_feat0 = item["edge_feat0"]
    values = item["values"]
    ...
    atom_graph = case.kind == "atom"
    grad_out = torch.randn((case.num_segments, 384), device=device)

    eager_f = torch_attention_macro_template(
        node_feat,
        edge_feat0,
        values,
        source_weight,
    ...
    cuda_f = cuda_fused_attention_macro_template(
        node_feat,
        edge_feat0,
        values,
        source_weight,
    ...
    eager_b = autograd_grads(
        torch_attention_macro_template,
        node_feat,
        edge_feat0,
        values,
        source_weight,
    ...
    cuda_b = autograd_grads(
        cuda_fused_attention_macro_template,
        node_feat,
        edge_feat0,
        values,
        source_weight,
    ...
    common_args = (node_feat, edge_feat0, values, source_weight, target_weight, source_index, target_index, out_template)
    forward_timings = {
        "torch_eager": cuda_time_ms(lambda: torch_attention_macro_template(*common_args), warmup=args.warmup, iters=args.iters),
        "torch_compile": cuda_time_ms(lambda: compiled_eval(*common_args), warmup=args.warmup, iters=args.iters),
        "cuda_fused": cuda_time_ms(
            lambda: cuda_fused_attention_macro_template(*common_args, atom_graph),
            warmup=args.warmup,
            iters=args.iters,
        ),
    }
    fwd_bwd_timings = {
        "torch_eager": cuda_time_ms(
            lambda: autograd_grads(torch_attention_macro_template, *common_args, grad_out),
            warmup=args.warmup,
            iters=args.iters,
        ),
        "torch_compile": cuda_time_ms(
            lambda: autograd_grads(compiled_grad, *common_args, grad_out),
            warmup=args.warmup,
            iters=args.iters,
        ),
        "cuda_fused": cuda_time_ms(
            lambda: autograd_grads(cuda_fused_attention_macro_template, *common_args, grad_out, atom_graph=atom_graph),
            warmup=args.warmup,
            iters=args.iters,
        ),
    }
    return {
        ...
        "errors_vs_torch_eager": {
            "torch_compile_fusion_node_feat": rel_error(eager_f, compile_f),
            "cuda_fused_fusion_node_feat": rel_error(eager_f, cuda_f),
            "torch_compile_grad_node_feat": rel_error(eager_b[0], compile_b[0]),
            "torch_compile_grad_edge_feat0": rel_error(eager_b[1], compile_b[1]),
            "torch_compile_grad_values": rel_error(eager_b[2], compile_b[2]),
            "cuda_fused_grad_node_feat": rel_error(eager_b[0], cuda_b[0]),
            "cuda_fused_grad_edge_feat0": rel_error(eager_b[1], cuda_b[1]),
...
def main() -> None:
    ...
    compiled_eval = torch.compile(torch_attention_macro_eval_template, dynamic=True, mode=args.torch_compile_mode)
    compiled_grad = torch.compile(torch_attention_macro_grad_template, dynamic=True, mode=args.torch_compile_mode)

    collect_start = time.perf_counter()
    cases, collection_meta = collect_cases(args)
    collect_s = time.perf_counter() - collect_start
    if not cases:
        raise SystemExit(f"No attention macro cases were collected. Collection metadata: {collection_meta}")

    results = []
    for case in tqdm(cases, desc="benchmark attention macro cases"):
        result = benchmark_case(compiled_eval, compiled_grad, case, args)
    ...
    payload = {
        "metadata": {
            ...
            "torch_compile": {
                ...
                "scope": "local alpha_projection + attention_reduce + node_update_input_concat only",
            ...
            "baseline_mapping": {
                "torch_eager": "PyTorch alpha matmul + segment softmax weighted sum + torch.cat",
                "torch_compile": "torch.compile(torch_eager_macro, dynamic=True), local function only",
                "cuda_fused": "PyTorch alpha matmul + _CudaFusedLineAttention + torch.cat",
            },
        },
