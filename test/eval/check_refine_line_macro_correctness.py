...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="refine_line macro correctness harness")
    ...
    parser.add_argument("--candidate", default="r1")
...
def prepare_inputs(calculator, graph, block_index: int, task: str, disabled_env: dict[str, None]):
    ...
    layer = calculator.model.interaction_block[block_index].refine_block_line_graph
    layer.profile_prefix = f"interaction_block.{block_index}.refine_line"
    return {
        "layer": layer,
        "graph": batch_graph["line_graph_dict"],
        "node_feat": attn_edge.detach(),
        "edge_feat": attn_threebody.detach(),
        "smooth": smooth_weight["line graph"].detach(),
        "atom_feat": attn_node.detach(),
    }


def run_layer(layer, graph: dict[str, Any], node, edge, smooth, atom):
    return layer(node, edge, smooth, graph, None, atom)
...
def check_one(calculator, graph, graph_id: int, args: argparse.Namespace):
    ...
    if prepared is None:
        return {"graph_id": int(graph_id), "skipped": True, "reason": "empty_line_graph"}

    layer = prepared["layer"]
    line_graph = prepared["graph"]
    bases = [prepared["node_feat"], prepared["edge_feat"], prepared["smooth"], prepared["atom_feat"]]

    ref_inputs = [base.detach().clone().requires_grad_(True) for base in bases]
    with patched_env(disabled_env):
        ref_node_out, ref_edge_out = run_layer(layer, line_graph, *ref_inputs)

    cand_inputs = [base.detach().clone().requires_grad_(True) for base in bases]
    with patched_env({**disabled_env, **candidate_env}):
        cand_node_out, cand_edge_out = run_layer(layer, line_graph, *cand_inputs)
    ...
    comparisons = {
        ...
        "grad_atom": tensor_diff(ref_grads[3], cand_grads[3], atol=args.atol, rtol=args.rtol),
    }
    return {
...
def main() -> None:
    ...
    results = []
    for graph_id in tqdm(keys, desc="refine_line_correctness", leave=False):
    ...
    for key in ("node_out", "edge_out", "grad_node", "grad_edge", "grad_smooth", "grad_atom"):
