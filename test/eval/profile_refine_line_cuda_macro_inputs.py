...
STAGE_RE = re.compile(r"interaction_block\.(\d+)\.refine_line\.(.+)")
TARGET_STAGES = {
    "gather_concat",
    "smooth_gather_mul",
    "edge_nonlinear_update",
    "edge_update_fused_first",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile refine_line under the current P108 best path for R-CUDA planning."
    )
    ...
    parser.add_argument("--output-dir", default="results/refine_line_profile_p108_best_20260525")
...
def numeric_summary(values: list[float]) -> dict[str, float]:
    if not values:
        return {"count": 0, "mean": 0.0, "std": 0.0, "min": 0.0, "p50": 0.0, "p90": 0.0, "max": 0.0}
...
def make_routing_hint(kernel_profile: dict[str, Any], stage_summary: dict[str, Any]) -> str:
    ...
    smooth = (
        by_stage.get("smooth_gather_mul", {}).get("kernel_ms", 0.0)
    ...
    if edge_update + gather >= max(smooth, ffn):
        return "Routing hint: R-CUDA1 is aligned with this profile; gather_concat + edge_nonlinear_update dominate refine_line attribution."
    if smooth >= max(edge_update + gather, ffn):
        return "Routing hint: smooth multiply/reduce dominates; consider R-CUDA2 before widening R-CUDA1."
    if ffn > 0.0:
        return "Routing hint: FFN input-grad is prominent; R-CUDA3 may be needed, but only after checking R-CUDA1/2 feasibility."
    refine_backward = stage_summary.get("module_backward.refine_line_ms", {}).get("mean", 0.0)
    return f"Routing hint: fine-grained stage attribution is weak; use whole refine_line backward mean {refine_backward:.3f} ms and kernel ranking first."


def write_markdown(payload: dict[str, Any], output_path: Path) -> None:
    lines = [
        "# refine_line P108 Profile",
        "",
    ...
    lines.extend(["", "## Top Kernels By refine_line Stage", ""])
...
def main() -> None:
    ...
    with torch.profiler.profile(activities=activities, record_shapes=False, profile_memory=False) as prof:
        for graph_id in profile_keys:
            try:
                with torch.profiler.record_function(f"refine_line_probe.graph_{int(graph_id)}"):
    ...
    compact = {
        ...
        "refine_line_stage_by_kernel": payload["kernel_profile"]["by_stage"],
